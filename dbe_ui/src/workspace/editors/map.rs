                ...
                .body_unindented(|ui| {
                    ...
                    egui::Grid::new(field_name).striped(true).show(ui, |ui| {
                        for (idx, (key, val)) in values.iter_mut().enumerate() {
                            ...
                            ui.push_id((&key, "key"), |ui| {
                                ...
                                if let Some(edited) = edited_key.as_mut().filter(|e| e.index == idx)
                                {
                                    ...
                                    if &edited.old_key != key {
                                        *edited_key = None;
                                        ...
                                        ui.horizontal(|ui| {
                                            ...
                                            let key_in_use = used_keys.contains(&edited.new_key);
                    ...
                    ui.horizontal(|ui| {
                        ui.push_id("add-new_key", |ui| {
                            let mut new_key = ui
                                .use_state(|| key_ty.default_value(ctx.registry).into_owned(), ())
                                .into_var();
